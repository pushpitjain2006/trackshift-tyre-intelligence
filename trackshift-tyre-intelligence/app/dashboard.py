...
from src.analysis.llm import stream_ai_summary
...
def main():
    ...
    if st.session_state.get(run_key, False):
        ...
        # ── SECTION 5.5: AI RACE ENGINEER SUMMARY ──
        st.markdown("""
        <div class="section-header">
            <h2>🤖 AI Race Engineer Summary</h2>
            <p>Automated intelligence report generated by Qwen via OpenRouter</p>
        </div>
        """, unsafe_allow_html=True)
        
        # We only want to stream this once per run, so we don't re-trigger it constantly.
        # But for hackathon simplicity, we can just use st.write_stream directly.
        # It will re-run on page interaction, which is fine for a demo.
        with st.chat_message("assistant", avatar="🤖"):
            st.write_stream(
                stream_ai_summary(
                    driver=driver,
                    stint_label=stint_label,
                    compound=compound,
                    raw_laps=preprocess_summary.raw_count,
                    valid_laps=preprocess_summary.valid_count,
                    naive_rate=getattr(naive_result, 'degradation_rate_per_lap', getattr(naive_result, 'rate_per_lap', 0)),
                    multi_rate=getattr(multi_result, 'degradation_rate_per_lap', getattr(multi_result, 'rate_per_lap', 0)),
                    final_rate=bayesian_result.rate_per_lap,
                    total_deg=bayesian_result.total_degradation,
                    model_used=model_used,
                    sampling_time=bayesian_result.sampling_time_sec,
                )
            )
